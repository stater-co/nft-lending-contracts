    ...
    TickMathTest
{
    ...
    constructor(
        ...
        require(
            _uniswapV3NftAddress != address(0),
            "A valid uniswap v3 address is required"
        );
        require(
            _lendingPool != address(0),
            "A valid lending pool address is required"
        );
    ...
    function setGlobalRouteVariables(
        address[] memory _whitelistedCurrencies,
        bool[] memory _status
    ) external onlyOwner {
    ...
    function _positionBalance(uint256 positionId, bool isToken1)
        internal
        view
        returns (uint256)
    {
    ...
    function _positionBalance(uint256 positionId)
        internal
        view
        returns (uint256)
    {
    ...
    function lackOfPayment(uint256 loanId) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < loan.nftTokenIdArray.length; ++i) {
            healthFactor +=
                (_positionBalance(loan.nftTokenIdArray[i]) *
                    liquidationTreshold) /
                loanControlPanel.amountDue;
        }

        return healthFactor / loan.nftTokenIdArray.length;
    }

    function getLoanAssetsValue(HealthFactorCreateLoanMethodParams memory loan)
        external
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < loan.nftTokenIdArray.length; ++i) {
            (, , address token0, address token1, , , , , , , , ) =
                INonfungiblePositionManager(uniswapV3NftAddress).positions(
                    loan.nftTokenIdArray[i]
                );
            require(
                whitelistedCurrencies[token0] && whitelistedCurrencies[token1],
                "Pair of tokens not accepted"
            );
    ...
    function createLoan(HealthFactorCreateLoanMethodParams memory loan)
        external
    {
        require(
            loan.nrOfInstallments > 0,
            "Loan must have at least 1 installment"
        );
        require(
            loan.nftTokenIdArray.length > 0,
            "Loan must have at least 1 NFT"
        );
        ...
        require(
            loan.loanAmount <= (loans[id].assetsValue / 100) * ltv,
            "The LTV must be under 60%"
        );
        ...
        loanControlPanels[id].amountDue =
            (loans[id].loanAmount * (loanFeesHandler[id].interestRate + 100)) /
            100; // interest rate >> 20%
        loans[id].nrOfInstallments = loan.nrOfInstallments;
        loanControlPanels[id].installmentAmount = loanControlPanels[id]
            .amountDue %
            loan.nrOfInstallments >
            0
            ? loanControlPanels[id].amountDue / loan.nrOfInstallments + 1
            : loanControlPanels[id].amountDue / loan.nrOfInstallments;
    ...
    function approveLoanWithPool(uint256 loanId, uint256 poolId)
        external
        payable
    {
        approveLoanCoreMechanism(loanId);
    ...
    function payLoan(uint256 loanId, uint256 amount) external payable {
        ...
        uint256 interestPerInstallement =
            (paidByBorrower * loanFeesHandler[loanId].interestRate) / 100; // entire interest for installment
        uint256 discount = calculateDiscount(loanId, msg.sender);
        uint256 interestToStaterPerInstallement =
            (interestPerInstallement *
                loanFeesHandler[loanId].interestRateToStater) / 100;

        if (discount != 1) {
            if (loans[loanId].currency == address(0)) {
                require(
                    payable(msg.sender).send(
                        interestToStaterPerInstallement / discount
                    )
                );
        ...
        loanControlPanels[loanId].paidAmount =
            loanControlPanels[loanId].paidAmount +
            paidByBorrower;
        loans[loanId].nrOfPayments =
            loans[loanId].nrOfPayments +
            (paidByBorrower / loanControlPanels[loanId].installmentAmount);
    ...
    function terminateLoan(uint256 loanId) external {
        ...
        require(
            (block.timestamp >= loanControlPanels[loanId].startEnd[1] ||
                loanControlPanels[loanId].paidAmount >=
                loanControlPanels[loanId].amountDue) &&
                lackOfPayment(loanId) > 3
        );
        require(
            loanControlPanels[loanId].status == Status.LIQUIDATED ||
