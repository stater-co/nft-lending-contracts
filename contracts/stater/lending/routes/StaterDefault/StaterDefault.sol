...
contract StaterDefault is Ownable, LendingCore, DefaultCreateLoanMethod {
    ...
    function lackOfPayment(uint256 loanId) public view returns (bool) {
        return
            ...
            loanControlPanels[loanId].startEnd[0] +
                (loans[loanId].nrOfPayments *
                    (loans[loanId].installmentTime /
                        loans[loanId].nrOfInstallments)) <=
            block.timestamp -
                (loanControlPanels[loanId].defaultingLimit *
                    (loans[loanId].installmentTime /
                        loans[loanId].nrOfInstallments));
    }

    // Borrower creates a loan
    function createLoan(DefaultCreateLoanMethodParams memory loan) external {
        require(
            loan.nrOfInstallments > 0 &&
                loan.loanAmount > 0 &&
                loan.nftAddressArray.length > 0
        );
        require(
            loan.nftAddressArray.length == loan.nftTokenIdArray.length &&
                loan.nftTokenIdArray.length == loan.nftTokenTypeArray.length
        );
    ...
    function editLoan(
        ...
        loanControlPanels[loanId].amountDue =
            ...
            100;
        loanControlPanels[loanId].installmentAmount = loanControlPanels[loanId]
            .amountDue %
            nrOfInstallments >
            0
            ? (loanControlPanels[loanId].amountDue / nrOfInstallments) + 1
            : loanControlPanels[loanId].amountDue / nrOfInstallments;
    ...
    function payLoan(uint256 loanId, uint256 amount) external payable {
        ...
        uint256 interestToStaterPerInstallement =
            (interestPerInstallement * interestRateToStater) / 100;
    ...
    function terminateLoan(uint256 loanId) external {
        ...
        require(
            (block.timestamp >= loanControlPanels[loanId].startEnd[1] ||
                loanControlPanels[loanId].paidAmount >=
                loanControlPanels[loanId].amountDue) || lackOfPayment(loanId)
        );
        require(
            loanControlPanels[loanId].status == Status.LIQUIDATED ||
                loanControlPanels[loanId].status == Status.APPROVED
    ...
    function updateLoan(
        ...
        loanFeesHandler[loanId].interestRateToStater = feesHandler
