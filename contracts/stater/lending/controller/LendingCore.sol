...
import "../../plugins/StaterTransfers/StaterTransfers.sol";
...
contract LendingCore is StaterTransfers {
    ...
    function _percent(uint256 numerator, uint256 denominator)
        internal
        pure
        returns (uint256)
    {
     ...
     */
    function calculateDiscount(uint256 loanId, address onBehalfOf)
        internal
        view
        returns (uint256)
    {
        return
            StaterDiscounts(loanControlPanels[loanId].discountsHandler)
                .calculateDiscount(onBehalfOf);
    }
}
