...
contract LendingData is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard {
  ...
  uint256 public installmentTimeLevel = 5; // weeks
  ...
  function setGlobalVariables(uint256 _ltv,uint256 _installmentFrequency,uint256 _installmentTimeLevel,uint256 _interestRate,uint256 _interestRateToStater) external onlyOwner {
    ltv = _ltv;
  ...
  function generateInstallmentFrequency() public view returns(uint256){
      if ( installmentTimeLevel == 1 ){
        return installmentFrequency.mul(1 seconds);
      }else if ( installmentTimeLevel == 2 ){
        return installmentFrequency.mul(1 minutes);  
      }else if ( installmentTimeLevel == 3 ){
        return installmentFrequency.mul(1 hours);
      }else if ( installmentTimeLevel == 4 ){
