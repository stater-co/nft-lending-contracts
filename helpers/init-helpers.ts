...
export const initReservesByHelper = async (
  ...
  stableDebtTokenImplementationAddress = await (
    await deployGenericStableDebtToken()
  ).address;
  variableDebtTokenImplementationAddress = await (
    await deployGenericVariableDebtToken()
  ).address;
  ...
  if (delegatedAwareReserves.length > 0) {
    const delegationAwareATokenImplementation =
      await deployDelegationAwareATokenImpl(verify);
    delegationAwareATokenImplementationAddress =
      delegationAwareATokenImplementation.address;
