...
export const usingPolygon = () =>
  DRE &&
  Object.keys(ePolygonNetwork).includes(
    (DRE as HardhatRuntimeEnvironment).network.name
  );
...
export const verifyAtPolygon = async (
  ...
  */
  ...
  const cleanedSourceCode = removeLines(
    removeLines(
      removeLines(flattenSourceCode, LICENSE_IDENTIFIER, 1),
      SOLIDITY_PRAGMA,
      1
    ),
  ...
  try {
    ...
    if (response.status === 200 && response.data.message === "OK") {
      console.log(
        `[Polygon Verify] Verified contract at Matic ${net} network.`
      );
      console.log(
