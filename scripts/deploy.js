...
async function main() {
  ...
  const discounts = await Discounts.deploy({ nonce: BigNumber.from(initialNonce).add(100) });
  ...
  await discounts.addDiscount(2,"0x1a08a4be4c59d808ee730d57a369b1c09ed62352",20,[], { nonce: BigNumber.from(initialNonce).add(200) });
  console.log('Discounts: Add Geyser Discount >> ' + JSON.stringify(discounts));
  await discounts.addDiscount(1,"0x4100670ee2f8aef6c47a4ed13c7f246e621228ec",2,[1,4,5,2], { nonce: BigNumber.from(initialNonce).add(300) });
  ...
  const promissoryNote = await PromissoryNote.deploy("Test Promissory Note","TPN", { nonce: BigNumber.from(initialNonce).add(400) });
  ...
  const lendingMethods = await LendingMethods.deploy({ nonce: BigNumber.from(initialNonce).add(500) });
  ...
  const lendingTemplate = await LendingTemplate.deploy(promissoryNote.address,lendingMethods.address,discounts.address,{ nonce: BigNumber.from(initialNonce).add(600) });
  ...
  await deployedPromissoryNote.setLendingDataAddress(lendingTemplate.address,{ nonce: BigNumber.from(initialNonce).add(200) });
