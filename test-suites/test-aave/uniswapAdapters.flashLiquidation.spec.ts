...
makeSuite("Uniswap adapters", (testEnv: TestEnv) => {
  ...
  const depositAndHFBelowOne = async () => {
    ...
    const userGlobalDataBefore = await pool.getUserAccountData(
      borrower.address
    );
  ...
  const depositSameAssetAndHFBelowOne = async () => {
    ...
    const userGlobalDataBefore2 = await pool.getUserAccountData(
      borrower.address
    );

    const amountWETHToBorrow = new BigNumber(
      userGlobalDataBefore2.availableBorrowsETH.toString()
    )
      .multipliedBy(0.8)
      .toFixed(0);
  ...
  describe("Flash Liquidation Adapter", () => {
    ...
    describe("executeOperation: succesfully liquidateCall and swap via Flash Loan with profits", () => {
      it("Liquidates the borrow with profit", async () => {
        ...
        const liquidatorWethBalanceBefore = await weth.balanceOf(
          liquidator.address
        );

        // Set how much ETH will be sold and swapped for DAI at Uniswap mock
        await (
          await mockUniswapRouter.setAmountToSwap(weth.address, expectedSwap)
        ).wait();
        ...
        const expectedProfit = ethers.BigNumber.from(
        ...
        const liquidatorWethBalanceAfter = await weth.balanceOf(
          liquidator.address
        );
        ...
        const borrowAssetContractBalance = await dai.balanceOf(
          flashLiquidationAdapter.address
        );
        ...
        expect(
          daiReserveDataAfter.liquidityIndex.toString()
        ).to.be.bignumber.gte(
    ...
    describe("executeOperation: succesfully liquidateCall with same asset via Flash Loan, but no swap needed", () => {
      it("Liquidates the borrow with profit", async () => {
        ...
        const expectedCollateralLiquidated = new BigNumber(
          ...
          .div(
            new BigNumber(assetPrice.toString()).times(
        ...
        await expect(Promise.resolve(tx)).to.not.emit(
          flashLiquidationAdapter,
          "Swapped"
        );
        ...
        const borrowAssetContractBalance = await weth.balanceOf(
          flashLiquidationAdapter.address
        );
    ...
    describe("executeOperation: succesfully liquidateCall and swap via Flash Loan without profits", () => {
      it("Liquidates the borrow", async () => {
        ...
        const collateralAssetContractBalance = await dai.balanceOf(
          flashLiquidationAdapter.address
        );
        const borrowAssetContractBalance = await weth.balanceOf(
          flashLiquidationAdapter.address
    ...
    describe("executeOperation: succesfully liquidateCall all available debt and swap via Flash Loan ", () => {
      it("Liquidates the borrow", async () => {
        ...
        const amountToLiquidate = userReserveDataBefore.currentStableDebt
          .div(2)
          .toFixed(0);
        const extraAmount = new BigNumber(amountToLiquidate)
          .times("1.15")
          .toFixed(0);
    ...
    describe("executeOperation: invalid params", async () => {
      ...
      it("Revert if debt asset amount to liquidate is greater than requested flash loan", async () => {
        ...
        const amountToLiquidate =
          userReserveDataBefore.currentStableDebt.div(2);
