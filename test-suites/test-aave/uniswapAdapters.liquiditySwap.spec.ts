  ...
  describe("UniswapLiquiditySwapAdapter", () => {
    ...
    describe("executeOperation", () => {
      ...
      it("should correctly swap tokens and deposit the out tokens in the pool", async () => {
        ...
        await mockUniswapRouter.setAmountToReturn(
          weth.address,
          expectedDaiAmount
        );
        ...
        await aWETH
          .connect(user)
          .approve(uniswapLiquiditySwapAdapter.address, liquidityToSwap);
        const userAEthBalanceBefore = await aWETH.balanceOf(userAddress);

        // Subtract the FL fee from the amount to be swapped 0,09%
        const flashloanAmount = new BigNumber(liquidityToSwap.toString())
          .div(1.0009)
          .toFixed(0);
      ...
      it("should correctly swap and deposit multiple tokens", async () => {
        ...
        const amountUSDCtoSwap = await convertToCurrencyDecimals(
          usdc.address,
          "10"
        );
        ...
        await pool
          .connect(user)
          .deposit(usdc.address, amountUSDCtoSwap, userAddress, 0);
        ...
        await mockUniswapRouter.setAmountToReturn(
          weth.address,
          expectedDaiAmountForEth
        );
        await mockUniswapRouter.setAmountToReturn(
          usdc.address,
          expectedDaiAmountForUsdc
        );

        await aWETH
          .connect(user)
          .approve(uniswapLiquiditySwapAdapter.address, amountWETHtoSwap);
        const userAEthBalanceBefore = await aWETH.balanceOf(userAddress);
        await aUsdc
          .connect(user)
          .approve(uniswapLiquiditySwapAdapter.address, amountUSDCtoSwap);
        ...
        expect(adapterDaiBalance).to.be.eq(Zero);
        expect(adapterDaiAllowance).to.be.eq(Zero);
        expect(userADaiBalance).to.be.eq(
          expectedDaiAmountForEth.add(expectedDaiAmountForUsdc)
        );
        expect(userAEthBalance).to.be.lt(userAEthBalanceBefore);
        expect(userAEthBalance).to.be.gte(
      ...
      it("should revert if caller not lending pool", async () => {
        const { users, weth, oracle, dai, aWETH, uniswapLiquiditySwapAdapter } =
          testEnv;
        const user = users[0].signer;
        const userAddress = users[0].address;
      ...
      it("should work correctly with tokens of different decimals", async () => {
        ...
        await mockUniswapRouter
          .connect(user)
          .setAmountToReturn(usdc.address, expectedDaiAmount);
        ...
        await aUsdc
          .connect(user)
          .approve(uniswapLiquiditySwapAdapter.address, aUsdcBalance);
        // Subtract the FL fee from the amount to be swapped 0,09%
        const flashloanAmount = new BigNumber(amountUSDCtoSwap.toString())
          .div(1.0009)
          .toFixed(0);
    ...
    describe("swapAndDeposit", () => {
      ...
      it("should revert if inconsistent params", async () => {
        const { users, weth, dai, uniswapLiquiditySwapAdapter, oracle } =
          testEnv;
        const user = users[0].signer;
