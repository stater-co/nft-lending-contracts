...
makeSuite("Uniswap adapters", (testEnv: TestEnv) => {
  ...
  describe("UniswapRepayAdapter", () => {
    beforeEach(async () => {
      ...
      const usdcLiquidity = await convertToCurrencyDecimals(
        usdc.address,
        "2000000"
      );
    ...
    describe("executeOperation", () => {
      it("should correctly swap tokens and repay debt", async () => {
        ...
        const amountWETHtoSwap = await convertToCurrencyDecimals(
          weth.address,
          "10"
        );
        ...
        await aWETH
          .connect(user)
          .approve(uniswapRepayAdapter.address, liquidityToSwap);
        const userAEthBalanceBefore = await aWETH.balanceOf(userAddress);

        await mockUniswapRouter
          .connect(user)
          .setAmountToSwap(weth.address, liquidityToSwap);
        ...
        const userDaiStableDebtAmount = await daiStableDebtContract.balanceOf(
          userAddress
        );
      ...
      it("should correctly swap tokens and repay debt with permit", async () => {
        ...
        const ownerPrivateKey = require("../../test-wallets.js").accounts[1]
          .secretKey;
        if (!ownerPrivateKey) {
      ...
      it("should revert if caller not lending pool", async () => {
        const { users, pool, weth, aWETH, oracle, dai, uniswapRepayAdapter } =
          testEnv;
        const user = users[0].signer;
        const userAddress = users[0].address;
      ...
      it("should revert if there is not debt to repay with the specified rate mode", async () => {
        const { users, pool, weth, oracle, dai, uniswapRepayAdapter, aWETH } =
          testEnv;
        const user = users[0].signer;
        const userAddress = users[0].address;
        ...
        await weth
          .connect(user)
          .transfer(uniswapRepayAdapter.address, amountWETHtoSwap);
      ...
      it("should revert when max amount allowed to swap is bigger than max slippage", async () => {
        const { users, pool, weth, oracle, dai, aWETH, uniswapRepayAdapter } =
          testEnv;
        const user = users[0].signer;
        const userAddress = users[0].address;
        ...
        await aWETH
          .connect(user)
          .approve(uniswapRepayAdapter.address, bigMaxAmountToSwap);

        await mockUniswapRouter
          .connect(user)
          .setAmountToSwap(weth.address, bigMaxAmountToSwap);
      ...
      it("should swap, repay debt and pull the needed ATokens leaving no leftovers", async () => {
        ...
        await mockUniswapRouter
          .connect(user)
          .setAmountToSwap(weth.address, actualWEthSwapped);
        ...
        expect(userAEthBalance).to.be.eq(
          userAEthBalanceBefore.sub(actualWEthSwapped)
        );
      ...
      it("should correctly swap tokens and repay the whole stable debt", async () => {
        ...
        await mockUniswapRouter
          .connect(user)
          .setAmountToSwap(weth.address, amountWETHtoSwap);
      ...
      it("should correctly repay debt via flash loan using the same asset as collateral", async () => {
        const { users, pool, aDai, dai, uniswapRepayAdapter, helpersContract } =
          testEnv;
        const user = users[0].signer;
        const userAddress = users[0].address;
        ...
        await pool
          .connect(user)
          .borrow(dai.address, debtAmount, 2, 0, userAddress);
        ...
        await aDai
          .connect(user)
          .approve(uniswapRepayAdapter.address, flashLoanDebt);
        ...
        const adapterADaiBalance = await aDai.balanceOf(
          uniswapRepayAdapter.address
        );
        const userDaiBalance = await dai.balanceOf(userAddress);

        expect(adapterADaiBalance).to.be.eq(
          Zero,
          "adapter aDAI balance should be zero"
        );
        expect(adapterDaiBalance).to.be.eq(
          Zero,
          "adapter DAI balance should be zero"
        );
    ...
    describe("swapAndRepay", () => {
      ...
      it("should revert if there is not debt to repay", async () => {
        const { users, weth, aWETH, oracle, dai, uniswapRepayAdapter } =
          testEnv;
        const user = users[0].signer;
      ...
      it("should correctly repay debt using the same asset as collateral", async () => {
        const { users, pool, dai, uniswapRepayAdapter, helpersContract, aDai } =
          testEnv;
        const user = users[0].signer;
        const userAddress = users[0].address;
        ...
        await aDai
          .connect(user)
          .approve(uniswapRepayAdapter.address, amountCollateralToSwap);
        const userADaiBalanceBefore = await aDai.balanceOf(userAddress);
        const userDaiBalanceBefore = await dai.balanceOf(userAddress);
