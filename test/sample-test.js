...
let discounts, erc721, erc1155, lendingMethods, lendingTemplate, erc20;
...
describe("Smart Contracts Setup", function () {
  ...
  it("Should deploy the lending methods", async function () {
    ...
    const _lendingMethods = await LendingMethods.deploy("Test","T");
  ...
  it("Should deploy the lending template", async function () {
    ...
    const _lendingTemplate = await LendingTemplate.deploy("Test","T");
  ...
  for ( let i = 1 , l = nrOfWorkflowsToTest; i <= l; ++i ) {
    ...
    if ( willEdit ) {
      ...
      it("Edit loan " + i, async function () {
        ...
        const operation = await lendingTemplate.editLoan([
          i,
          loanValue,
          nrOfInstallments,
          currency,
