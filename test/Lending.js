...
contract('LendingData', (accounts) => {
  ...
  it('Should create a loan', async () => {
	...
	for ( let i = 0 , l = nrOfTokensToAdd ; i < l ; ++i ){
		...
		for ( let j = 0 , k = createToken.logs.length ; j < k ; ++j )
			if ( createToken.logs[j].event === "Transfer" ){
				...
				let approveTokenToContract = await tokenInstance.approve(instance.address,tokenId,{ from : accounts[0] });
	...
	let createLoan = await instance.createLoan(loanAmount,nrOfInstallments,currency,assetsValue,nftAddressArray,nftTokenIdArray,creationId,{ from : accounts[0] });
  ...
  it('Should find a lender for loan', async () => {
	...
	toPayForApprove = web3.utils.hexToNumber(web3.utils.toHex(toPayForApprove));
    const approveLoan = await instance.approveLoan(loansCount-1,{ from : accounts[1] , value : toPayForApprove });
  ...
  it('Should set the global ltv', async () => {
	const instance = await LendingData.deployed();
    const ltv = await instance.setLtv(600,{ from : accounts[0] });
  ...
  it('Should pay for loan, 1x installment', async () => {
	...
	installmentAmount = web3.utils.hexToNumber(web3.utils.toHex(installmentAmount));
    const payLoan = await instance.payLoan(loansCount-1,{ from : accounts[0] , value : installmentAmount });
