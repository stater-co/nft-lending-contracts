...
task("verify:general", "Verify contracts at Etherscan")
  ...
  .setAction(async ({ all, pool }, localDRE) => {
    ...
    const lendingPoolConfiguratorAddress =
      await addressesProvider.getLendingPoolConfigurator(); //getLendingPoolConfiguratorProxy();
    ...
    const lendingPoolConfiguratorProxy = await getProxy(
      lendingPoolConfiguratorAddress
    );
    const lendingPoolCollateralManagerProxy = await getProxy(
      lendingPoolCollateralManagerAddress
    );
    ...
    if (all) {
      ...
      const lendingPoolConfiguratorImpl = notFalsyOrZeroAddress(
